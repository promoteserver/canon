...
<div class="row ">
<div class="col-xs-8 collapse navbar-collapse navbar-ex1-collapse center "> 
...
<div id="bars" >
<?php
	$sqlfp = "SELECT product FROM feature ORDER BY id DESC LIMIT 0 , 4"; // select 4 featured products.
	$featured_result= mysql_query($sqlfp); // execute select 3 featured products.
	while($fp = mysql_fetch_array($featured_result))// Featured Products
	{
		$sql1 = "SELECT * FROM products where fid = '".$fp['product']."' ORDER BY products.`idpd` ASC LIMIT 0 , 1 "; // Gimme the products who are featured
		$product_result = mysql_query($sql1); // the query
		$p = mysql_fetch_array($product_result); // the array of products who are featured
		$sql= "SELECT * FROM photos where idsec='".$p['fid']."' order by id asc";
		$result= mysql_query($sql);
		$row = mysql_fetch_array($result); // Array with featured photos
		if($i >= 2) // this is for the responsive things. 
		{
			echo '<div class="hidden-xs col-sm-4 col-lg-3 top20 col-xxs-12 text-center"> 
			 <a href="#" class="col-img"><img src="products_img/'.$row['small'].'" width="250" height="150" class="img-border img-responsive" ><p class="lead">George II Mahogany Sideboard Server Buffet </p></a>
			</div>';
		}else{
			echo ' <div class="col-xs-6 col-sm-4 col-lg-3 top20 col-xxs-12 text-center"> 
			 <a href="#" class="col-img"><img src="products_img/'.$row['small'].'" width="250" height="150" align="bottom" class="img-border img-responsive" ><p class="lead">French Louis XV Bureau Du Roi Roll Top Desk Inlay </p></a>
			 </div>';
		}
...
<div class="text-center clear-both" style="background:#FFCD09; height:10px"></div> 
...
<div class="col-xs-12 col-sm-12 col-lg-padding-left-0 col-lg-padding-right-0">
 ...
 <div class="col-xs-12 col-sm-4 col-lg-3 hidden-xs col-lg-padding-left-0"> 
...
<div class="col-xs-12 text-center container-fluid back-footercopy" > 
